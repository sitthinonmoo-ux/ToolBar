...
// files, then move them into FiveM's plugins folder. See fivem-reshade-addon plugin
// for the second half (the F8 / CitizenFX.ini [Addons] step).
...
function relocateToFiveMPlugins(gameDir, proxyDll, fivemAppDir, context) {
  ...
  return new Promise((resolve) => {
    ...
    child.on('exit', async (code) => {
      ...
      try {
        ...
        const { moved, pluginsDir } = relocateToFiveMPlugins(gameDir, proxyDll, fivemAppDir, context);
